...
-- update company name
update settings_changes set value='--- LightBulb\n...\n' where key='/server/company';
...
-- set company name
  IF NOT EXISTS (select key,value from settings_changes where key='/server/company') THEN
    insert into settings_changes (key,value,created_at,updated_at,resource_type,resource_id) VALUES ('/server/company','--- LightBulb\n...\n',now(),now(),'MiqServer',(select id from miq_servers));
  END IF;
